...
export function generateReceiptPdf(receipt: ReceiptForPdf): ArrayBuffer {
  const doc = new jsPDF();
  const pageW = doc.internal.pageSize.getWidth();

  const recipientName    = receipt.careHomeName    ?? receipt.user.companyName ?? receipt.user.name ?? receipt.user.email ?? "";
  const recipientAddress = receipt.careHomeAddress ?? receipt.user.address ?? "";
